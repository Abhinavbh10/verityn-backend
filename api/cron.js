  ...
  if (hour === 5) {
    ...
    for (const combo of WARM_COMBINATIONS) {
      try {
        ...
        const res = await fetch(`${VERCEL_URL}/api/news?country=${combo.country}&category=${combo.category}&max=10`, {
    ...
    for (const country of DIGEST_COUNTRIES) {
      try {
        const res = await fetch(`${VERCEL_URL}/api/digest?country=${country}`);
        if (res.ok) {
    ...
    for (const country of DIGEST_COUNTRIES) {
      try {
        const res = await fetch(`${VERCEL_URL}/api/briefing?country=${country}`);
        if (res.ok) {
